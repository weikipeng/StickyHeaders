...
public class StickyHeaderLayoutManager extends RecyclerView.LayoutManager {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        FOpenLog.e("wikiFocus debug onLayoutChildren ==== 0");
        if (adapter == null) {
            return;
        }

        if (scrollTargetAdapterPosition >= 0) {
            FOpenLog.e("wikiFocus debug onLayoutChildren ==== scrollTargetAdapterPosition >= 0");
            ...
            FOpenLog.e("wikiFocus debug onLayoutChildren ==== pendingSavedState != null && pendingSavedState.isValid()");
            ...
            FOpenLog.e("wikiFocus debug onLayoutChildren ==== updateFirstAdapterPosition");
            updateFirstAdapterPosition();
        }
        ...
        if (firstViewAdapterPosition > state.getItemCount()) {
            FOpenLog.e("wikiFocus debug onLayoutChildren ==== firstViewAdapterPosition > state.getItemCount()");
            firstViewAdapterPosition = 0;
        }

        FOpenLog.e("wikiFocus debug onLayoutChildren ==== firstViewAdapterPosition:"
                + firstViewAdapterPosition + " state.getItemCount():" + state.getItemCount() + "\n\n\n-----");
        ...
        for (int adapterPosition = firstViewAdapterPosition; adapterPosition < state.getItemCount(); adapterPosition++) {
            ...
            if (itemViewType == SectioningAdapter.TYPE_HEADER) {
                FOpenLog.e("wikiFocus debug onLayoutChildren ==== itemViewType: TYPE_HEADER");
                headerViews.add(v);
                ...
                FOpenLog.e("wikiFocus debug onLayoutChildren ==== itemViewType: TYPE_GHOST_HEADER");
                ...
                FOpenLog.e("wikiFocus debug onLayoutChildren ==== itemViewType: normal");
                height = getDecoratedMeasuredHeight(v);
                layoutDecorated(v, left, top, right, top + height);
            }
            ...
            if (v.getBottom() >= parentBottom) {
                FOpenLog.e("wikiFocus debug onLayoutChildren ==== v.getBottom() >= parentBottom break;");
                break;
            }
        }
        ...
        if (totalVendedHeight < innerHeight) {
            FOpenLog.e("wikiFocus debug onLayoutChildren ==== totalVendedHeight < innerHeight\n\n\n");
            ...
            FOpenLog.e("wikiFocus debug onLayoutChildren ==== updateHeaderPositions\n\n\n");
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        FOpenLog.e("wikiFocus debug scrollVerticallyBy ==== \n\n\nstart-------");
        if (getChildCount() == 0) {
            return 0;
        }
        ...
        int left     = getPaddingLeft();
        int right    = getWidth() - getPaddingRight();
        FOpenLog.e("wikiFocus debug scrollVerticallyBy ====firstViewAdapterPosition:" + firstViewAdapterPosition
                + " left:" + left + " right:" + right + " dy:" + dy);
        if (dy < 0) {
            ...
            while (scrolled > dy) {
                ...
                if (firstViewAdapterPosition > 0 && scrolled > dy) {
                    FOpenLog.e("wikiFocus debug scrollVerticallyBy ==== firstViewAdapterPosition > 0 && scrolled > dy" +
                            "\n firstViewAdapterPosition:" + firstViewAdapterPosition);
                    ...
                    FOpenLog.e("wikiFocus debug scrollVerticallyBy ==== isHeader:" + isHeader +
                            "\n firstViewAdapterPosition:" + firstViewAdapterPosition);
                    // skip the header, move to next item above
                    if (isHeader) {
                        firstViewAdapterPosition--;
                        if (firstViewAdapterPosition < 0) {
                            FOpenLog.e("wikiFocus debug scrollVerticallyBy ==== firstViewAdapterPosition < 0 break");
                            break;
                        }
                        ...
                        if (isHeader) {
                            FOpenLog.e("wikiFocus debug scrollVerticallyBy ==== still isHeader break");
                            break;
                        }
                    }
                    ...
                    if (isGhostHeader) {
                        ...
                        FOpenLog.e("wikiFocus debug scrollVerticallyBy ==== isGhostHeader top:" + top + " bottom:" + bottom);
                    } else {
                        measureChildWithMargins(v, 0, 0);
                        top = bottom - getDecoratedMeasuredHeight(v);
                        FOpenLog.e("wikiFocus debug scrollVerticallyBy ==== not isGhostHeader top:" + top + " bottom:" + bottom);
                    }

                    FOpenLog.e("wikiFocus debug scrollVerticallyBy ==== layoutDecorated left:" + left + " top:" + top + " right:" +
                            right + " bottom:" + bottom);
            ...
            while (scrolled < dy) {
                ...
                if (scrolled < dy && nextAdapterPosition < state.getItemCount()) {
                    FOpenLog.e("wikiFocus debug scrollVerticallyBy ==== scrolled < dy nextAdapterPosition < state.getItemCount()" +
                            "\n nextAdapterPosition:" + nextAdapterPosition + " state.getItemCount():" + state.getItemCount());

                    int top = getDecoratedBottom(bottomView);
                    FOpenLog.e("wikiFocus debug scrollVerticallyBy ==== top:" + top);

                    int itemViewType = adapter.getItemViewBaseType(nextAdapterPosition);
                    if (itemViewType == SectioningAdapter.TYPE_HEADER) {
                        FOpenLog.e("wikiFocus debug scrollVerticallyBy ==== TYPE_HEADER");
                        ...
                        FOpenLog.e("wikiFocus debug scrollVerticallyBy ==== TYPE_GHOST_HEADER");
                        ...
                        FOpenLog.e("wikiFocus debug scrollVerticallyBy ==== type_normal");
    ...
    private void updateHeaderPositions(RecyclerView.Recycler recycler) {
        FOpenLog.e("\n\n\nwikiFocus debug updateHeaderPositions ==== \n\n\n--------start");
        ...
        FOpenLog.e("wikiFocus debug updateHeaderPositions ==== header is always positioned at top" +
        ...
        for (View headerView : headerViews) {
            ...
            for (int i = 0, n = getChildCount(); i < n; i++) {
                ...
                if (isViewRecycled(view)) {
                    FOpenLog.e("wikiFocus debug updateHeaderPositions ==== child view " + i + " has been recycled continue");
                    continue;
                }

                int type = getViewBaseType(view);
                if (type == SectioningAdapter.TYPE_HEADER) {
                    FOpenLog.e("wikiFocus debug updateHeaderPositions ==== child view " + i + " TYPE_HEADER continue");
                    continue;
                }
                ...
                if (viewSectionIndex == sectionIndex) {
                    if (type == SectioningAdapter.TYPE_GHOST_HEADER) {
                        ghostHeader = view;
                        FOpenLog.e("wikiFocus debug updateHeaderPositions ==== child view " + i + " TYPE_GHOST_HEADER viewSectionIndex:"
                                + viewSectionIndex + " sectionIndex:" + sectionIndex);
                    }
                } else if (viewSectionIndex == sectionIndex + 1) {
                    if (firstViewInNextSection == null) {
                        firstViewInNextSection = view;
                        FOpenLog.e("wikiFocus debug updateHeaderPositions ==== firstViewInNextSection " + i
                        +" viewSectionIndex:"+viewSectionIndex+" sectionIndex:"+sectionIndex);
                    }
                }
            }
            ...
            if (ghostHeader != null) {
                ...
                if (ghostHeaderTop >= top) {
                    ...
                    FOpenLog.e("wikiFocus debug updateHeaderPositions ==== HeaderPosition.NATURAL ");
                }
            }
            ...
            if (firstViewInNextSection != null) {
                ...
                if (nextViewTop - height < top) {
                    ...
                    FOpenLog.e("wikiFocus debug updateHeaderPositions ==== HeaderPosition.TRAILING ");
                }
            }
            ...
            layoutDecorated(headerView, left, top, right, top + height);
            FOpenLog.e("wikiFocus debug updateHeaderPositions ==== (" + left + "," + top + "," + right + "," + (top + height) + ")");
            // notify adapter of positioning for this header
            recordHeaderPositionAndNotify(sectionIndex, headerView, headerPosition);
            FOpenLog.e("wikiFocus debug updateHeaderPositions ==== recordHeaderPositionAndNotify sectionIndex:"
                    +sectionIndex+" headerPosition:"+headerPosition);
        }

        FOpenLog.e("wikiFocus debug updateHeaderPositions ==== \n\n\n---------end");
    }
